...
import { loadPageImageFromFile, parseJSONSafe } from '@/utils/common'
...
export const useRegionStore = defineStore('region', () => {
  ...
  const regions = computed<Region[]>(() => currentScheme.value?.regions || [])
  ...
  const sortedRegions = computed<Region[]>(() =>
    [...regions.value].sort((a, b) => a.order - b.order)
  )
  ...
  async function loadPageImage(file: File): Promise<void> {
  ...
  function importScheme(jsonStr: string): ValidationResult & { schemeId?: string } {
    const parsed = parseJSONSafe<SplitScheme>(jsonStr)
    if (!parsed.valid || !parsed.data) {
      return { valid: false, message: parsed.message || 'JSON解析失败' }
    }

    let validation: ValidationResult
    try {
      validation = validateSplitScheme(parsed.data)
    } catch (e) {
    ...
    try {
      ...
      return {
        valid: true,
        message: `已导入方案「${scheme.name}」(${scheme.regions.length}个区域)`,
        schemeId: scheme.id
      }
    } catch (e) {
  ...
  function addRegion(position: RegionPosition, category: RegionCategory): ValidationResult {
  ...
  function updateRegion(
    ...
    if (updates.name !== undefined) {
      const nameValidation = validateRegionName(updates.name, regionId, currentScheme.value.regions)
  ...
  function runAutoRecognition(): ValidationResult {
    ...
    if (templates.value.length === 0)
      return { valid: false, message: '暂无可用模板，请先从已有区域保存模板' }
